...
from mujoco_playground._src.locomotion.g1_23dof import base as g1_base
from mujoco_playground._src.locomotion.g1_23dof import g1_constants as consts
...
class G1Locomotion(g1_base.G1Env):
  ...
  def __init__(
    ...
    super().__init__(
        xml_path=consts.get_xml().as_posix(),
        config=config,
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    DEBUG_NO_ARM_CONTROL = True
    if DEBUG_NO_ARM_CONTROL:
      action = action.at[self._arm_indices].set(0)
  ...
  def _get_reward(
    ...
    return {
        ...
        "joint_deviation_arm": self._cost_joint_deviation_arm(data.qpos[7:]),
  ...
  def _cost_joint_deviation_arm(self, qpos: jax.Array) -> jax.Array:
